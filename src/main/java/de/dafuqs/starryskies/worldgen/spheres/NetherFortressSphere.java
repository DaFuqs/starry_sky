...
public class NetherFortressSphere extends PlacedSphere {
	...
	public NetherFortressSphere(ConfiguredSphere<?> template, float radius, List<ConfiguredSphereDecorator<?, ?>> decorators, List<Pair<EntityType<?>, Integer>> spawns, ChunkRandom random,
								int shellRadius) {
	...
	public static class Config extends ConfiguredSphere<Config.Config> {
		...
		@Override
		public NetherFortressSphere generate(ChunkRandom random, DynamicRegistryManager registryManager) {
			int shellRadius = Support.getRandomBetween(random, this.minShellRadius, this.maxShellRadius);
			return new NetherFortressSphere(this, randomBetween(random, minSize, maxSize), selectDecorators(random), selectSpawns(random), random, shellRadius);
		}
