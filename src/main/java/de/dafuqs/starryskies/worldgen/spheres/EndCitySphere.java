...
public class EndCitySphere extends PlacedSphere {
	...
	public EndCitySphere(ConfiguredSphere<?> template, float radius, List<ConfiguredSphereDecorator<?, ?>> decorators, List<Pair<EntityType<?>, Integer>> spawns, ChunkRandom random, int shellRadius) {
	...
	public static class Template extends ConfiguredSphere<Template.Config> {
		...
		@Override
		public EndCitySphere generate(ChunkRandom random, DynamicRegistryManager registryManager) {
			int shellRadius = Support.getRandomBetween(random, minShellRadius, maxShellRadius);
			return new EndCitySphere(this, randomBetween(random, minSize, maxSize), selectDecorators(random), selectSpawns(random), random, shellRadius);
		}
