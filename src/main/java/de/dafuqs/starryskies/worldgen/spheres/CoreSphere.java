...
public class CoreSphere extends PlacedSphere {

	private final BlockStateProvider coreBlock;
	private final BlockStateProvider shellBlock;
	private float coreRadius;

	public CoreSphere(ConfiguredSphere<?> template, float radius, List<ConfiguredSphereDecorator<?, ?>> decorators, List<Pair<EntityType<?>, Integer>> spawns, ChunkRandom random,
	...
	public static class Template extends ConfiguredSphere<Template.Config> {
		...
		@Override
		public CoreSphere generate(ChunkRandom random, DynamicRegistryManager registryManager) {
			...
			return new CoreSphere(this, radius, selectDecorators(random), selectSpawns(random), random, coreBlock, shellBlock, coreRadius);
		}
