...
public class OceanMonumentSphere extends Sphere<OceanMonumentSphere.Config> {
	...
	@Override
	public PlacedSphere<?> generate(ConfiguredSphere<? extends Sphere<OceanMonumentSphere.Config>, Config> configuredSphere, Config config, ChunkRandom random, DynamicRegistryManager registryManager) {
		return new OceanMonumentSphere.Placed(configuredSphere, configuredSphere.getSize(random), configuredSphere.getDecorators(random), configuredSphere.getSpawns(random), random, config.coreRadius.get(random), config.shellThickness.get(random));
	}
