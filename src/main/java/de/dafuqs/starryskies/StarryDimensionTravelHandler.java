...
public class StarryDimensionTravelHandler {
	...
	public static RegistryKey<World> modifyNetherPortalDestination(@NotNull Entity thisEntity, RegistryKey<World> serverWorld) {
		if (thisEntity.getEntityWorld().getRegistryKey().equals(StarryDimensionKeys.OVERWORLD_KEY)) {
			return StarryDimensionKeys.NETHER_KEY;
		} else if (thisEntity.getEntityWorld().getRegistryKey().equals(StarryDimensionKeys.NETHER_KEY)) {
			return StarryDimensionKeys.OVERWORLD_KEY;
		}
		return serverWorld;
	}
	...
	public static boolean handleEndPortalCollision(BlockState state, World world, BlockPos pos, Entity entity) {
		if (world instanceof ServerWorld
			...
			if (StarryDimensionKeys.END_KEY.equals(sourceRegistryKey)) {
				destinationRegistryKey = StarryDimensionKeys.OVERWORLD_KEY;
			} else if (StarryDimensionKeys.OVERWORLD_KEY.equals(sourceRegistryKey)) {
				destinationRegistryKey = StarryDimensionKeys.END_KEY;
			} else {
	...
	public static @Nullable TeleportTarget handleGetTeleportTarget(@NotNull Entity thisEntity, ServerWorld destination) {
		boolean isTravellingToStarryEnd = thisEntity.getWorld().getRegistryKey() == StarryDimensionKeys.OVERWORLD_KEY && destination.getRegistryKey() == StarryDimensionKeys.END_KEY;
		boolean isTravellingBackFromStarryEnd = thisEntity.getWorld().getRegistryKey() == StarryDimensionKeys.END_KEY && destination.getRegistryKey() == StarryDimensionKeys.OVERWORLD_KEY;
		if (!isTravellingBackFromStarryEnd && !isTravellingToStarryEnd) {
			boolean isTravellingToStarryNether = thisEntity.getWorld().getRegistryKey() == StarryDimensionKeys.OVERWORLD_KEY && destination.getRegistryKey() == StarryDimensionKeys.NETHER_KEY;
			boolean isTravellingBackFromStarryNether = thisEntity.getWorld().getRegistryKey() == StarryDimensionKeys.NETHER_KEY && destination.getRegistryKey() == StarryDimensionKeys.OVERWORLD_KEY;
